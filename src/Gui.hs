...
renderClientScript guiData =
    unlines
        ...
        , "  const activeGoal = state.goals.find((goal) => Math.abs(goal.minute - minute) < 2.4);"
        , "  const ballReferenceX = activeGoal ? activeGoal.scorer.x : carrier.x;"
        , "  const ballReferenceY = activeGoal ? activeGoal.scorer.y : carrier.y;"
        ...
        , "    const progress = Math.min(1, Math.max(0, (minute - activeGoal.minute + 2.4) / 4.8));"
        ...
        , "  }"
        , "  const ballEase = activeGoal ? 0.105 : 0.055;"
